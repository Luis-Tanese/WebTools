...
const initDateTan = async () => {
    try {
        [locales, holidays] = await Promise.all([
            loadJSON("/assets/scripts/locales.json"),
            loadJSON("/assets/scripts/holidays.json"),
        ]);

        console.log("DateTan initialized successfully");
    } catch (error) {
        console.error("Failed to initialize DateTan:", error);
        locales = {
            "en-us": {
                days: [
                    "Sunday",
...
initDateTan().then(() => {
    window.dateTan = dateTan;
    window.dateTanParse = dateTanParse;

    document.dispatchEvent(new Event("dateTanReady"));
});
